...
#include <WS2tcpip.h>
#pragma comment(lib, "Ws2_32.lib")  //libary with socket info 

namespace servfunc
{

	const char* function1::echo(const char* message) {
		const int x = strlen(message);
		char *y = new char[x + 1];
	...
	const char* function1::reverse(const char* message) {
		const int x= strlen(message);						
		char *y = new char[x+1];							
		for (int i = 0; i <= (int)strlen(message); i++) {  
			y[i] = message[strlen(message)-(i+1)];			
		}
		y[strlen(message)] = '\0';
		return y;
	}
...
namespace Comms {
	...
	const SOCKET comms::setup(const char* IP, const char* PORT) {
		...
		ZeroMemory(&hint, sizeof(hint));  //put everything to 0
	...
	const int comms::snd(const SOCKET x, const char* message, const int len) {
		...
		while (prep) {
			...
			if (status > len) {				
				strncpy_s(b, len , &message[iter],len-1);
				iter += len - 1;								
				b[len - 1] = 0;
				status = 1;
